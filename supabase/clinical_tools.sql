...
-- Safe to rerun: it also adds missing columns if an older table already exists.
...
alter table public.fluid_calculators
    ...
    add column if not exists species text,
    add column if not exists notes text,
    alter column calculation_name set not null;
...
alter table public.species_toxicities
    ...
    add column if not exists notes text,
    alter column toxin set not null,
    alter column species set not null;
...
where not exists (
  ...
  where existing.drug_name = seed.drug_name
    and existing.species = seed.species
...
where not exists (
  select 1 from public.cri_protocols existing
  where existing.drug_name = seed.drug_name
    and coalesce(existing.indication, '') = coalesce(seed.indication, '')
);
...
where not exists (
  select 1 from public.emergency_drug_calculator existing
  where existing.drug_name = seed.drug_name
    and coalesce(existing.indication, '') = coalesce(seed.indication, '')
);
...
where not exists (
  select 1 from public.fluid_calculators existing
  where existing.calculation_name = seed.calculation_name
);
...
where not exists (
  select 1 from public.transfusion_calculators existing
  where existing.species = seed.species
);
...
where not exists (
  select 1 from public.species_toxicities existing
  where existing.toxin = seed.toxin
    and existing.species = seed.species
);
