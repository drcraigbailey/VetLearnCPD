...
const beforeInlineShare = source;
source = source.replace(/\n\n                \{shareOpen && \([\s\S]*?\n                \)\}/, '');
if (source !== beforeInlineShare) {
  changed = true;
  console.log('Removed inline User Tools share list.');
} else {
  console.log('Skipped inline User Tools share list removal: already patched or marker not found.');
}
