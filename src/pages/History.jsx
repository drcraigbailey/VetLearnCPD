...
export default function History({ user, darkMode = false }) {
  ...
  const buildSharedCpdMessage = (item) => {
    const parts = [
      ...
      getEntrySource(item) === "manual" ? "Time entry: Manual" : "Time entry: Timer",
                ...
                <div key={item.id} className={`${darkMode ? "bg-white/10 border-white/10" : "bg-white/90 border-[#DCEDEA]"} border rounded-lg p-5 shadow-[0_10px_24px_rgba(11,55,96,0.06)]`}>
                  <div className="flex justify-between gap-3">
                    <div>
                      ...
                      <span className={`mt-2 inline-flex rounded-full px-3 py-1 text-xs font-black ${source === "manual" ? "bg-[#E8F8F5] text-[#0B3760]" : "bg-[#71CFC2]/20 text-[#0F8F83]"}`}>
                        {source === "manual" ? "Manual time" : "Timer recorded"}
                      </span>
                    </div>
