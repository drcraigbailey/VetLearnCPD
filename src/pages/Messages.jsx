...
export default function Messages({ user, darkMode }) {
  ...
  const handleStartNewChat = async (colleague) => {
    ...
    let conversation = conversations.find(item => String(item.colleague?.id) === String(colleague.id));
    if (!conversation) {
  ...
  const tabClass = (tab) => `px-4 py-2 rounded-full whitespace-nowrap font-bold text-sm transition ${activeTab === tab ? "bg-[#71CFC2] text-[#062F63] shadow-md" : darkMode ? "bg-white/10 text-slate-300" : "bg-[#E8F8F5] text-[#0B3760]"}`;
          ...
          <div className={`w-full h-full ${panelClass}`}>
            <div className="p-5 border-b border-inherit">
              <div className="flex justify-between items-center mb-4">
                <h2 className={`text-2xl font-black ${textPrimary}`}>{isNewChatMode ? "Select Colleague" : "Inbox"}</h2>
                <button onClick={() => { setIsNewChatMode(!isNewChatMode); setSearchQuery(""); }} className={`p-3 rounded-full transition-colors ${isNewChatMode ? "bg-slate-200 text-slate-600" : "bg-[#E8F8F5] text-[#0F8F83]"}`} title={isNewChatMode ? "Cancel" : "New Message"}>
              ...
              <div className={`flex items-center px-4 py-3 rounded-xl border ${darkMode ? "bg-black/20 border-white/10" : "bg-slate-50 border-slate-200"}`}>
                ...
                <input type="text" placeholder={isNewChatMode ? "Search network..." : "Search conversations..."} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-transparent border-none outline-none text-base w-full" />
              </div>
            </div>
              ...
              {isNewChatMode ? (
                filteredColleagues.length === 0 ? <div className="p-8 text-center opacity-50 text-sm">No colleagues found in your network.</div> : filteredColleagues.map(colleague => (
                  <button key={colleague.id} onClick={() => handleStartNewChat(colleague)} className="w-full text-left p-5 border-b border-inherit transition-colors flex items-center gap-4 hover:bg-black/5 dark:hover:bg-white/5">
                    <div className="h-12 w-12 rounded-full bg-[#E8F8F5] text-[#0F8F83] flex items-center justify-center shrink-0 font-bold text-lg">{colleague?.full_name?.charAt(0) || <User size={20} />}</div>
                ...
                return (
                  <div key={chat.id} className="w-full p-5 border-b border-inherit transition-colors flex items-center gap-4 hover:bg-black/5 dark:hover:bg-white/5">
                    <button onClick={() => setActiveChat(chat)} className="flex-1 min-w-0 text-left flex items-center gap-4">
                      <div className={`${isUnread ? "bg-[#71CFC2] text-[#0B3760]" : darkMode ? "bg-white/10 text-slate-300" : "bg-[#E8F8F5] text-[#0B3760]"} h-12 w-12 rounded-full flex items-center justify-center shrink-0 font-bold text-lg`}>{chat.colleague?.full_name?.charAt(0) || <User size={20} />}</div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className={`${isUnread ? "font-semibold opacity-90" : "opacity-60"} text-sm truncate`}>{chat.lastMsg?.sender_id === user.id && "You: "}{chat.lastMsg?.content || "No messages yet"}</div>
                      </div>
                    </button>
          ...
          <div className={`w-full h-full animate-in fade-in zoom-in-95 duration-200 ${panelClass}`}>
            <div className={`p-5 border-b flex justify-between items-center z-10 ${darkMode ? "border-white/10" : "border-slate-100"}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="h-11 w-11 rounded-full bg-[#E8F8F5] text-[#0F8F83] flex items-center justify-center font-black text-lg shadow-inner">{activeChat.colleague?.full_name?.charAt(0) || <User size={18} />}</div>
                <div>
              ...
              {chatItems.map(item => {
                ...
                return (
                  <div key={item.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                    <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm shadow-sm ${isMe ? "bg-[#71CFC2] text-[#0B3760] font-medium rounded-br-sm" : darkMode ? "bg-white/10 text-white rounded-bl-sm" : "bg-white border border-slate-100 text-[#113247] rounded-bl-sm"}`}>{item.content && <div className="whitespace-pre-wrap">{item.content}</div>}</div>
            ...
            <form onSubmit={handleSend} className={`p-4 border-t ${darkMode ? "bg-[#071A24] border-white/10" : "bg-white border-slate-100"}`}>
              <div className={`flex gap-3 items-center rounded-lg p-3 border ${darkMode ? "bg-white/10 border-white/10" : "bg-[#F0F6F5] border-[#DCEDEA]"}`}>
                ...
                <button type="submit" disabled={!newMessage.trim() || sending} className="h-11 w-11 rounded-lg bg-[#71CFC2] text-[#0B3760] flex items-center justify-center disabled:opacity-40 disabled:grayscale transition-all shrink-0">{sending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} className="ml-1" />}</button>
              </div>
            </form>
          </div>
        )}
