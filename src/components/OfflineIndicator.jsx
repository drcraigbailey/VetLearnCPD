      ...
      >
        ...
        <span className="min-w-0">
          ...
          <span className={`block text-xs font-semibold leading-tight ${darkMode ? 'text-slate-300' : 'text-[#0F8F83]'}`}>
            Saved/offline content will still open where available.
          </span>
        </span>
        <span className="relative ml-1 flex h-3 w-3 shrink-0" aria-hidden="true">
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#71CFC2] opacity-70" />
          <span className="relative inline-flex h-3 w-3 rounded-full bg-[#71CFC2]" />
        </span>
      </div>
    </div>
  )
